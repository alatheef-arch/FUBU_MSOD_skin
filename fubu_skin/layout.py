...
# provided by the main app's execution scope when this code is loaded.
...
def register_callbacks():
    ...
    def update_skin_final_zone_grid(packaged_data, trigger):
        ...
        pivoted = df_raw.pivot_table(index=STRINGER_PITCH_COLUMN_ID, columns="Frame Pitch (ID)", values="Skin Thickness (mm)", aggfunc="first", sort=False)
        ...
        if stored_panels:
            for panel in stored_panels:
                for coord in panel.get("coords", []):
                    conditional_styles.append({"if": {"row_index": coord["row"], "column_id": coord["column_id"]}, "backgroundColor": panel.get("color"), "color": panel.get("text_color")})
    ...
    def save_skin_properties(
        ...
        for coord in panel_to_update.get("coords", []):
            ...
            skin_mask = ((df_skin_final["Zone Name"] == zone_to_edit) & (df_skin_final["Row"] == stringer_pitch) & (df_skin_final["Column"] == frame_pitch))
            if skin_mask.any():
